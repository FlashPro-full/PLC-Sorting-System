...
function appendHistory({ barcode, location, label }) {
    ...
    item.innerHTML = `<strong>${barcode}</strong> → ${label} (slot ${location}) <span>${timestamp}</span>`;
    historyList.prepend(item);

    while (historyList.childElementCount > historyLimit) {
...
async function updateLocation() {
    ...
    try {
        ...
        if (data.available && data.location) {
            ...
            locationInput.style.backgroundColor = "rgba(58, 122, 254, 0.2)";
            setTimeout(() => {
                locationInput.style.backgroundColor = "";
            }, 500);
            return data.location;
        }
        // No location available - this is normal, don't log as error
        return null;
    } catch (error) {
...
async function scanBarcode() {
    ...
    if (!barcode) {
        ...
        if (resultDiv) {
            resultDiv.innerText = "Error: Please enter a barcode.";
        }
        return;
    }

    // Location is optional (original behavior - can be omitted)
    // If no location provided, try to get from queue, but don't require it
    if (!location) {
        console.log("📍 No location provided, fetching from queue...");
        location = await updateLocation();
        if (location) {
            console.log("✅ Location fetched from queue:", location);
        } else {
            console.log("⚠️ No location available from queue - will send without location (original behavior)");
        }
    }

    // scan_id is optional - original code worked without it
    const payload = { scan: barcode };
    if (location) {
        payload.scan_id = location;
    }
    console.log("📤 Sending POST to /scan:", payload);
    ...
    if (!response.ok) {
        ...
        console.error("💡 Note: Backend automatically processes barcodes from scanner. Manual submission requires photo eye location.");
        alert(`⚠️ ${errorMsg}\n\nBackend automatically processes barcodes from scanner.\nManual testing requires photo eye location to be available.`);
        return;
    }

    // Update result display if it exists
    if (resultDiv) {
        resultDiv.innerText = JSON.stringify(result, null, 2);
    }

    // Update the scanned barcode display field
    const scannedBarcodeDisplay = document.getElementById('scannedBarcodeDisplay');
    if (scannedBarcodeDisplay) {
        scannedBarcodeDisplay.value = barcode;
        // Add visual feedback - briefly highlight
        scannedBarcodeDisplay.style.backgroundColor = "rgba(58, 122, 254, 0.2)";
        scannedBarcodeDisplay.style.borderColor = "#3a7afe";
        setTimeout(() => {
            scannedBarcodeDisplay.style.backgroundColor = "#f8f9fa";
            scannedBarcodeDisplay.style.borderColor = "var(--border)";
        }, 500);
        console.log(`✅ Barcode ${barcode} displayed in frontend`);
    }

    const label =
        result?.winner?.winnerSubModule ||
        result?.winner?.winnerModule ||
        result?.meta?.product_group ||
        "Unknown";

    appendHistory({ barcode, location, label });

    // Trigger 3D visualization event
    if (result.routing) {
        const event = new CustomEvent('itemScanned', {
            detail: {
                barcode: barcode,
                location: location,
                pusher: result.routing.pusher,
                label: result.routing.label
            }
        });
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (!barcodeInput) {
        console.error("❌ barcodeInput element not found in DOM!");
        return;
    }
    
    console.log("✅ barcodeInput element found");
    ...
    // Frontend input field is for MANUAL TESTING ONLY
    // Backend handles barcode scanning automatically from serial scanner
    // For keyboard mode, backend should use separate terminal (like legacy code)
    // This input field is just for manual testing/debugging
    
    barcodeInput.addEventListener("input", (event) => {
        const currentValue = barcodeInput.value.trim();
        // Visual feedback - highlight input when typing
        barcodeInput.style.borderColor = "#3a7afe";
        barcodeInput.style.backgroundColor = "rgba(58, 122, 254, 0.05)";
        
        // Reset input styling after a delay
        setTimeout(() => {
            barcodeInput.style.borderColor = "";
            barcodeInput.style.backgroundColor = "";
        }, 200);
    ...
    // Only allow manual submission via Enter key (for testing)
    // Backend handles automatic barcode processing from scanner
    barcodeInput.addEventListener("keypress", (event) => {
        if (event.key === "Enter") {
            console.log("⏎ Enter key pressed - manual submission (for testing only)");
            const barcode = barcodeInput.value.trim();
            if (barcode) {
                console.log("📦 Manual barcode submission:", barcode);
                scanBarcode(); // Manual submission only
            }
        }
    });

    // Note: Location updates are handled via barcode input event
    // No need for periodic polling - location is fetched on-demand when barcode is entered

    focusBarcode();
    console.log("✅ Barcode input field focused and ready");
